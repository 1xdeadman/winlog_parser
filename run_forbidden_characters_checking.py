...
@click.command()
@click.option("--fieldnames", '-f', default="data/column_names_utf-8", help="Путь с именем  до файла с именами колонок", required=True)
@click.option("--logs_file", '-l', default='data/Security.evtx', help="Путь с именем  до файла логов", required=True)
@click.option("--result_file", '-r', default='data/result.csv', help="Путь с именем для сохранения файла csv", required=True)
@click.option("--columns_file", '-c', default='data/check_forbidden_characters.log', help="Путь с именем для сохранения подсчитанных по колонкам запретных символов", required=True)
def run(fieldnames: str, logs_file: str, result_file: str, columns_file: str):
    full_start = time()
